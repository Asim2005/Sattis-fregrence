...
// Parse the URI path to determine the route
$uri    = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
$uri    = str_replace('/api', '', $uri); // strip /api prefix if present

// Dynamically handle subdirectories (find 'backend' and take everything after it)
$parts  = explode('/', trim($uri, '/'));
$backendIndex = array_search('backend', $parts);

if ($backendIndex !== false) {
    $parts = array_slice($parts, $backendIndex + 1);
}

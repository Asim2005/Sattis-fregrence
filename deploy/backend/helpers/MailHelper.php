...
class MailHelper {
    // Holds the last error string so callers can surface it
    ...
    public static function send(string $toEmail, string $subject, string $htmlBody, string $altBody = ''): bool {
        ...
        // From email must be on the same domain as the SMTP account to pass SPF.
        // Fall back to the SMTP username itself if not explicitly set.
        ...
        if (empty($host) || empty($username)) {
            self::$lastError = 'SMTP is not configured. Go to Admin → Settings and fill in SMTP Host and Username, then save before testing.';
            error_log("MailHelper: " . self::$lastError);
            return false;
        }
        ...
        try {
            ...
            // Envelope sender must match the authenticated SMTP account for SPF compliance
            ...
            error_log("MailHelper: Message could not be sent. Mailer Error: " . self::$lastError);
            return false;
        }
    }
    ...
    public static function sendAdminOrderNotification(array $order, array $items): bool {
        $settings = self::getSettings();
        $adminEmail = !empty($settings['admin_notify_email'])
            ? $settings['admin_notify_email']
            : (!empty($settings['smtp_username']) ? $settings['smtp_username'] : $settings['contact_email'] ?? '');
    ...
    public static function sendOrderStatusNotification(array $order, string $status, ?string $signupEmail = null): bool {
        $settings = self::getSettings();
        $supportEmail = !empty($settings['contact_email']) ? $settings['contact_email'] : ($settings['smtp_username'] ?? 'support@sattis.store');
        ...
        $htmlBody = "
            <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px; color: #333;'>
                ...
                <p style='margin-top: 30px;'>If you have any questions, please contact our support team at {$supportEmail}.</p>
